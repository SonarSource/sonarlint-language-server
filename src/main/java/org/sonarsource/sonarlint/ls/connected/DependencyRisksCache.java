...
public class DependencyRisksCache {
  ...
  private final Map<URI, List<ScaIssue>> dependencyRisksPerConfigScope = new ConcurrentHashMap<>();

  public void clear(URI fileUri) {
    dependencyRisksPerConfigScope.remove(fileUri);
  }

  public Optional<ScaIssue> getDependencyRiskById(String issueId) {
  ...
  public Stream<Diagnostic> getAsDiagnostics(URI folderUri) {
    return dependencyRisksPerConfigScope.getOrDefault(folderUri, emptyList())
      .stream()
      .filter(i -> i.getStatus().equals(ScaIssueDto.Status.OPEN))
      .flatMap(i -> DependencyRisksCache.convert(i).stream());
  }

  static Optional<Diagnostic> convert(ScaIssue issue) {
  ...
  static String message(ScaIssueDto issue) {
    return issue.getPackageName().concat(" ").concat(issue.getPackageVersion());
  }

  public void reload(URI folderUri, List<ScaIssue> scaIssues) {
    dependencyRisksPerConfigScope.put(folderUri, scaIssues);
  }

  public void add(URI folderUri, ScaIssue scaIssue) {
    dependencyRisksPerConfigScope.get(folderUri).add(scaIssue);
  }

  public void removeScaIssue(String folderUriStr, String key) {
    var folderUri = URI.create(folderUriStr);
    var issues = dependencyRisksPerConfigScope.get(folderUri);
    if (issues != null) {
      var issueToRemove = issues.stream().filter(scaIssue -> scaIssue.getId().toString().equals(key)).findFirst();
      issueToRemove.ifPresent(issues::remove);
    }
  }

  public Map<URI, List<ScaIssue>> getDependencyRisksPerConfigScope() {
